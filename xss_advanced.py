...
def stored_xss_scanner(url: str, params: str = "", check_url: str = "") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai stored XSS scan pada {url}")
...
def dom_xss_scanner(url: str) -> str:
    """
    Scan untuk DOM-based XSS menggunakan Playwright (headless browser).
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai DOM XSS scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def jsonp_injection_scanner(url: str) -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai JSONP injection scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
