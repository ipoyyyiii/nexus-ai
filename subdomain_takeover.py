...
def detect_subdomain_takeover(subdomain: str) -> str:
    ...
    try:
        ...
        return f"[+] {subdomain}: Tidak memiliki CNAME record yang mengarah ke external service (Aman)."
    ...
    if not matched_provider:
        return f"[+] {subdomain}: Memiliki CNAME ke {cname_target}, tapi tidak cocok dengan signature takeover yang diketahui."
    ...
    try:
        ...
        return f"[+] {subdomain}: Mengarah ke {matched_provider} ({cname_target}), tetapi layanannya tampaknya aktif / sudah diklaim."
        
    except requests.RequestException:
        return f"[-] {subdomain}: Gagal memvalidasi HTTP response untuk CNAME {cname_target} (Server down / RTO)."