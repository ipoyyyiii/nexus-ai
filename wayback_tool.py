...
def wayback_scraper(domain: str) -> str:
    ...
    yang udah dihapus dari UI tapi servernya masih aktif.
    """
    ...
    try:
        ...
        if not raw or len(raw) <= 1:
            return f"Wayback Machine tidak punya data historis untuk {domain}."

        # Skip header row
        entries = raw[1:]
        exec_logger.add_log("Wayback Scraper", "SUCCESS", f"Total URL historis ditemukan: {len(entries)}")
        ...
        if total_interesting == 0:
            output += "Tidak ada URL menarik yang ditemukan dari historical data.\n"

        return output

    except requests.Timeout:
        exec_logger.add_log("Wayback Scraper", "ERROR", "Timeout saat fetch CDX API")
        return f"Timeout: Wayback Machine tidak merespons dalam 30 detik untuk {domain}."
    except Exception as e:
        exec_logger.add_log("Wayback Scraper", "ERROR", f"Scraping gagal: {str(e)}")
