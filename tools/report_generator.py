...
class ReportGenerator:
    ...
    def generate(self, finding: Dict[str, Any]) -> str:
        ...
        if oob_id:
            ...
            report.append(f"Callback Domain: {oob_id}.whoopbhapzham.my.id")
            report.append(f"OOB Server: 70.153.26.71")
            report.append(f"Poll Endpoint: http://70.153.26.71/logs")
            report.append(f"Auth Header: Authorization: maybeftankseed123")
            report.append("")
            report.append(f"# Poll command:")
            report.append(f'curl -H "Authorization: maybeftankseed123" http://70.153.26.71/logs | grep "{oob_id}"')
            report.append("```")
            report.append("")
    ...
    def generate_from_phase_results(
        ...
        report.append("## Executive Summary")
        report.append("")
        report.append("This report presents the findings from a comprehensive penetration test")
        report.append(f"conducted against `{target}`. The assessment covered reconnaissance,")
        report.append("vulnerability analysis, exploitation, and risk assessment phases.")
        report.append("")
        ...
        phase_names = {
            ...
            "eksekutor": "Exploitation",
            "assessor": "Risk Assessment",
        }

        for phase_key, phase_name in phase_names.items():
            if phase_key in phase_results and phase_results[phase_key]:
                report.append(f"## {phase_name}")
                report.append("")
                report.append(phase_results[phase_key])
                report.append("")
                report.append("---")
                report.append("")
        ...
        report.append("")
        report.append("This report was generated by Nexus AI Pentest Agent. All testing was performed")
        report.append("with authorization from the target organization.")
        report.append("")
        report.append(f"*Report generated on {self.generated_at}*")
