...
def scan_ssrf(url: str, canary_domain: str = "") -> str:
    ...
    if not canary_domain:
        from engines.oob_engine import oob_engine
        canary_domain = oob_engine.generate_subdomain("ssrf")
    ...
    for param in SSRF_PARAMS:
        ...
        for payload in ssrf_payloads[:3]:
            ...
            try:
                t_start = time.time()
                # Use stealth request with browser-like headers
                stealth_headers = stealth.get_browser_headers(test_url)
                resp = SESSION.get(test_url, headers=stealth_headers, allow_redirects=False, proxies=current_proxy, timeout=4)
    ...
    # ── Phase 4: OOB Blind SSRF (Private interactsh server) ───────────────────
    if not check_cancelled(logger):
        if logger:
        ...
        try:
            ...
            if oob_result.get("found"):
                findings.append({
                    ...
                    "poll_duration": oob_result.get("poll_duration", 0),
                ...
                findings.append({
                    "type": "OOB SSRF Test Error",
                    "severity": "INFO",
                    "error": oob_result.get("error", "Unknown error"),
                })
...
def scan_idor(url: str, cookies: str = "", auth_header: str = "") -> str:
    ...
    try:
        ...
        if current_proxy:
            proxy_router.remove_dead_proxy(current_proxy)
        return json.dumps({"error": "Failed request baseline karena kendala proxy.", "status": "error"})
    except Exception as e:
