...
def insecure_deserialization_scanner(url: str, cookies: str = "", headers_json: str = "") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai deserialization scan pada {url}")
    ...
    approved = require_approval(
        ...
        context="Mengirim serialized object payloads dan menganalisis response",
        risk="medium",
        exec_logger=logger,
    )
    if not approved:
...
def web_cache_poisoning_scanner(url: str) -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai cache poisoning scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def cache_deception_scanner(url: str, auth_cookies: str = "") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai cache deception scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def ssrf_advanced_scanner(url: str, upload_param: str = "file") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai advanced SSRF scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100">
  ...
  <image href="{internal_url}" width="100" height="100"/>
    ...
    result = {
        ...
        "findings": findings,
        "note": "OOB SSRF (blind) requires canary domain setup for complete detection"
    }
