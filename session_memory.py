...
SESSION MEMORY
===============
Agent bisa "ingat" findings dari session-session sebelumnya terhadap target
yang sama. Waktu lo mulai pentest target X lagi besok, agent load dulu
semua yang udah ditemukan sebelumnya — subdomain, vuln, endpoint, tech stack.

Disimpan di Supabase tabel `session_memory`:
...
memory_type values:
- "subdomain"    — subdomain yang udah ditemukan
- "tech_stack"   — tech stack yang terdeteksi
- "vuln"         — vulnerability yang udah ditemukan/dicoba
- "endpoint"     — endpoint/parameter yang udah ditemukan
