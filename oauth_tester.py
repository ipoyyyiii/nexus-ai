...
def _test_state_parameter(authorize_url: str, client_id: str) -> dict:
    """Test apakah state parameter divalidasi."""
    findings = []
    ...
    try:
        ...
        if resp.status_code == 302 and "state" not in resp.headers.get("Location", ""):
            findings.append({
                ...
                "severity": "High",
                "detail": "Server tidak memvalidasi/meminta state parameter — rentan CSRF pada OAuth flow"
            })
    except Exception:
        pass
        ...
        try:
            ...
            if resp.status_code == 302 and "evil.com" in location:
                findings.append({
                    ...
                    "severity": "Critical",
                    "payload": payload_url,
                    "detail": f"Server menerima redirect ke domain tidak terdaftar via {bypass_type}"
                })
...
def _test_pkce_missing(authorize_url: str, client_id: str) -> list:
    ...
    try:
        rate_limiter.wait(_domain_of(authorize_url))
        # Request tanpa code_challenge (PKCE) — harusnya ditolak kalau PKCE di-enforce
        resp = requests.get(
            authorize_url,
            params={
                "client_id": client_id,
        ...
        if resp.status_code in (200, 302):
            findings.append({
                ...
                "severity": "Medium",
                "detail": "Server tidak mewajibkan PKCE — authorization code rentan di-intercept pada public clients (SPA/mobile apps)"
            })
    except Exception:
        pass
...
def oauth_flow_tester(target_url: str, client_id: str = "", redirect_uri: str = "") -> str:
    """
    Menguji implementasi OAuth/SSO pada target untuk menemukan kerentanan umum:
    ...
    exec_logger.add_log("OAuth Tester", "START", f"Memulai OAuth flow testing pada {target_url}")
    ...
    if not oauth_endpoints:
        return f"[+] Tidak ditemukan OAuth/SSO endpoint standar pada {target_url}. Target mungkin pake custom auth flow atau tidak mengimplementasi OAuth."
    ...
    output += f"Endpoints ditemukan: {', '.join(oauth_endpoints)}\n"
    output += f"Authorize endpoint yang ditest: {authorize_endpoint}\n\n"

    if not all_findings:
        output += "[+] Tidak ditemukan kerentanan OAuth yang obvious. Manual testing tetap disarankan untuk edge cases.\n"
        return output
    ...
    output += f"\n⚠️  Manual verification diperlukan untuk konfirmasi semua findings di atas.\n"
