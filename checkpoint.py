...
def require_approval(action: str, context: str, risk: str = "high", exec_logger=None) -> bool:
    """
    Helper yang dipanggil dari dalam tool sebelum eksekusi aksi berisiko.
    ...
    if not job_id:
        if exec_logger:
            exec_logger.add_log(
                "HITL", "BLOCKED",
                f"Aksi '{action}' ditolak: gak ada job_id di context (kemungkinan dijalankan di luar API flow)."
            )
        return False
