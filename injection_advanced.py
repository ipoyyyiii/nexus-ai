...
def blind_sqli_scanner(url: str, params: str = "") -> str:
    ...
    Berguna ketika error-based SQLi tidak menampilkan error di response.
    params: comma-separated parameter names to test
    """
    tool_name = "Blind SQLi Scanner"
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai blind SQLi scan pada {url}")
...
def nosql_injection_scanner(url: str, params: str = "") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai NoSQL injection scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def ldap_injection_scanner(url: str, params: str = "") -> str:
    ...
    Sering ditemukan di enterprise apps yang pake LDAP authentication (Active Directory).
    params: comma-separated parameter names (biasanya username, user, email)
    """
    tool_name = "LDAP Injection Scanner"
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai LDAP injection scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def xpath_injection_scanner(url: str, params: str = "") -> str:
    ...
    XPath injection terjadi di aplikasi yang query XML data store menggunakan user input.
    params: comma-separated parameter names to test
    """
    tool_name = "XPath Injection Scanner"
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai XPath injection scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
