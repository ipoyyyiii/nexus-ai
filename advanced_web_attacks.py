...
def host_header_injection_scanner(url: str) -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai host header injection scan pada {url}")
...
def race_condition_scanner(url: str, method: str = "POST", body: str = "", headers_json: str = "", concurrency: int = 20) -> str:
    ...
    Mengirim banyak request secara concurrent untuk exploit timing windows.
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai race condition scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."

    approved = require_approval(
        action=f"Race Condition scan pada {url}",
        context=f"Mengirim {concurrency} concurrent {method} requests ke {url} secara bersamaan",
        risk="high",
        exec_logger=logger,
    )
    if not approved:
...
def file_upload_scanner(url: str, file_param: str = "file") -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai file upload scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def http_request_smuggling_scanner(url: str) -> str:
    ...
    Mengirim raw HTTP requests dengan conflicting Content-Length dan Transfer-Encoding.
    Detection berdasarkan timing dan response behavior.
    """
    tool_name = "HTTP Request Smuggling Scanner"
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai HTTP request smuggling scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
...
def websocket_security_scanner(url: str) -> str:
    ...
    logger = _logger()
    logger.add_log(tool_name, "START", f"Memulai WebSocket security scan pada {url}")
    if check_cancelled(logger): return "EKSEKUSI DIBATALKAN: job di-cancel oleh user."
