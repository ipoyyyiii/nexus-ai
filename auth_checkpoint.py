...
class AuthCheckpointStore:
    ...
    def request_auth(
        ...
        Return:
            ...
            atau None kalau timeout/ditolak
        """
...
def request_auth(
    ...
    Return auth_data dict atau None kalau ditolak/timeout.
    """
    ...
    if auth_data:
        if exec_logger:
            exec_logger.add_log(
                "AUTH", "SUCCESS",
                f"Auth diterima: mode={auth_data.get('mode', 'unknown')}",
                {"domain": domain}
            )
    else:
        if exec_logger:
            exec_logger.add_log(
                "AUTH", "BLOCKED",
                f"Auth ditolak atau timeout untuk {domain}"
            )
