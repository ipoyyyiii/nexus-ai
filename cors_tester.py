...
def cors_tester(target_url: str) -> str:
    """
    Menguji implementasi CORS pada target untuk menemukan misconfiguration:
    ...
    approved = require_approval(
        ...
        context="Mengirim request dengan berbagai Origin header untuk test CORS policy",
        risk="low",
    ...
    for origin, test_name in test_origins:
        ...
        try:
            ...
            exec_logger.add_log("CORS Tester", "WARNING", f"Test {test_name} gagal: {str(e)}")
            continue
    ...
    if not findings:
        output += "[✅] CORS policy tampak aman. Tidak ada origin bypass yang berhasil.\n"
        return output
    ...
    output += "⚠️  Manual verification diperlukan untuk konfirmasi impact sebenarnya.\n"
